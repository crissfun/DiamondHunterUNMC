// added new tilemap to overwrite default tileMap to overwrite user's input coordinate
// Changes populatesitem() to read from "itemsandcharacter.txt"
...
private void populateItems() {
	...
	item = new Item(tileMap);
	item.setType(Item.BOAT);
	item.setTilePosition(index.get(3),index.get(2));
	items.add(item);
}
